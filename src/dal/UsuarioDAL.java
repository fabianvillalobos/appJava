...
public class UsuarioDAL {
    ...
    public int verificarExistenciaUsuario(String nombreUsuario) throws SQLException, ClassNotFoundException {
        //sp_BUSCARUSERNAME
        Connection con = new Conexion().abrirOracle();
        CallableStatement cstmt = con.prepareCall("{CALL ontour.sp_BUSCARUSERNAME(?,?)}");
    ...
    public int AgregarUsuario(UsuarioDTO usuarioDTO) throws SQLException, ClassNotFoundException {
        Connection con = new Conexion().abrirOracle();
        if(verificarExistenciaUsuario(usuarioDTO.getLoginUsr())== 0){
